...
class OracleGru(object):
    def __init__(self, num_vocabulary, batch_size, emb_dim, hidden_dim, sequence_length, start_token, ):
        ...
        with tf.compat.v1.variable_scope('generator'):
            ...
            self.g_embeddings = tf.compat.v1.Variable(
                tf.compat.v1.random_normal([self.num_vocabulary, self.emb_dim], 0.0, 1.0, seed=123314154))
        ...
        self.h0 = tf.compat.v1.stack(self.h0)
        ...
        def _g_recurrence(i, x_t, h_tm1, gen_o, gen_x):
            ...
            gen_o = gen_o.write(i, tf.compat.v1.reduce_sum(
                tf.compat.v1.multiply(tf.compat.v1.one_hot(next_token, self.num_vocabulary, 1.0, 0.0), tf.compat.v1.nn.softmax(o_t)),
                1))  # [batch_size] , prob
        ...
        self.g_predictions = tf.compat.v1.transpose(
        ...
        self.pretrain_loss = -tf.compat.v1.reduce_sum(
            tf.compat.v1.one_hot(tf.compat.v1.to_int32(tf.compat.v1.reshape(self.x, [-1])), self.num_vocabulary, 1.0, 0.0) * tf.compat.v1.log(
                tf.compat.v1.reshape(self.g_predictions, [-1, self.num_vocabulary]))) / (self.sequence_length * self.batch_size)

        self.out_loss = tf.compat.v1.reduce_sum(
            tf.compat.v1.reshape(
                -tf.compat.v1.reduce_sum(
                    tf.compat.v1.one_hot(tf.compat.v1.to_int32(tf.compat.v1.reshape(self.x, [-1])), self.num_vocabulary, 1.0, 0.0) * tf.compat.v1.log(
                        tf.compat.v1.reshape(self.g_predictions, [-1, self.num_vocabulary])), 1
    ...
    def init_matrix(self, shape):
        return tf.compat.v1.random_normal(shape, stddev=1.0, seed=10)

    def create_recurrent_unit(self, params):

        self.Wz = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.emb_dim, self.hidden_dim], 0.0, 0.1, seed=111))
        self.Uz = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], 0.0, 0.1, seed=211))
        self.bz = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.hidden_dim, ], 0.0, 0.1, seed=311))

        self.Wr = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.emb_dim, self.hidden_dim], 0.0, 0.1, seed=112))
        self.Ur = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], 0.0, 0.1, seed=212))
        self.br = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.hidden_dim, ], 0.0, 0.1, seed=312))

        self.W = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.emb_dim, self.hidden_dim], 0.0, 0.1, seed=113))
        self.U = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.hidden_dim], 0.0, 0.1, seed=213))
        self.b = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.hidden_dim, ], 0.0, 0.1, seed=313))
        ...
        def gru_unit(x, hidden_memory_stack):
            previous_hidden_state = tf.compat.v1.unstack(hidden_memory_stack)
            # update gate
            Zt = tf.compat.v1.sigmoid(
                tf.compat.v1.matmul(x, self.Wz) +
                tf.compat.v1.matmul(previous_hidden_state, self.Uz) +
                self.bz
            )
            # reset gate
            rt = tf.compat.v1.sigmoid(
                tf.compat.v1.matmul(x, self.Wr) +
                tf.compat.v1.matmul(previous_hidden_state, self.Ur) +
                self.br
            )
            # new memory
            ht_ = tf.compat.v1.tanh(
                tf.compat.v1.matmul(x, self.W) +
                tf.compat.v1.multiply(rt, tf.compat.v1.matmul(previous_hidden_state, self.U))
            )

            current_hidden_state = tf.compat.v1.multiply(tf.compat.v1.subtract(1.0, Zt), ht_) + tf.compat.v1.multiply(Zt, previous_hidden_state)
            return tf.compat.v1.stack(current_hidden_state)


        return gru_unit

    def create_output_unit(self, params):
        self.Wo = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.hidden_dim, self.num_vocabulary], 0.0, 1.0, seed=12341))
        self.bo = tf.compat.v1.Variable(tf.compat.v1.random_normal([self.num_vocabulary], 0.0, 1.0, seed=56865246))
        params.extend([self.Wo, self.bo])

        def unit(hidden_memory_tuple):
            hidden_state = tf.compat.v1.unstack(hidden_memory_tuple)
