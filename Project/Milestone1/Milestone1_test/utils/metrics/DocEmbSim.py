...
class DocEmbSim(Metrics):
    ...
    def get_wordvec(self, file):
        ...
        with graph.as_default():
            ...
            # Input data.
            train_dataset = tf.compat.v1.placeholder(tf.compat.v1.int32, shape=[batch_size])
            train_labels = tf.compat.v1.placeholder(tf.compat.v1.int32, shape=[batch_size, 1])
            valid_dataset = tf.compat.v1.constant(self.valid_examples, dtype=tf.compat.v1.int32)
            ...
            softmax_biases = tf.compat.v1.Variable(tf.compat.v1.zeros([vocabulary_size]))

            # Model.
            # Look up embeddings for inputs.
            embed = tf.compat.v1.nn.embedding_lookup(embeddings, train_dataset)
            # Compute the softmax loss, using a sample of the negative labels each time.
            loss = tf.compat.v1.reduce_mean(
                tf.compat.v1.nn.sampled_softmax_loss(weights=softmax_weights, biases=softmax_biases, inputs=embed,
